...
export default function AdminHomePage() {
  const pendingApplications = sampleApplications.filter((item) => item.status === "pending" || item.status === "submitted").length;
    ...
    <div className="space-y-6">
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          ...
          <h3 className="mt-2 text-[clamp(30px,4vw,40px)] leading-none tracking-[-2px]">{sampleApplications.length}</h3>
          <p className="mt-1 font-mono text-caption text-muted">{pendingApplications} under review</p>
        </Card>
        <Card>
        ...
        <section className="overflow-hidden border border-primary bg-surface">
          ...
          <div className="text-caption">
            {sampleApplications.map((row) => (
              <div key={row.id} className="grid grid-cols-[1.2fr_0.9fr_1fr] gap-3 border-b border-divider px-5 py-3 font-mono">
                <span>{row.fullName}</span>
                <span className="uppercase">{row.status}</span>
                <span>{row.status === "accepted" || row.status === "rejected" ? "Archived" : "Accept / Reject"}</span>
              </div>
            ))}
          </div>
        </section>
